...
Based on this recommendation we continue with three variables `abdomen`,
`weight`, and `wrist`.  The model selected by Heinze, Wallisch, and
Dunkler (2018) had seven variables `height` (fixed), `abdomen` (fixed),
`wrist`, `age`, `neck`, `forearm`, and `chest`- Three first variables
are same except that projpred selected `weight` instead of `height`, so
decision to not fix variables seems to have been a good
choice. Replacement of `height` with `weight` is not surprising since
these variables do have similar information as seen also in the above
plot of joint posterior of `weight` and `height` in the full model.
...
In theory LOO-CV should have smaller variation than bootstrap, but in
practice the bootstrap results are quite similar. From 100 bootstrap
iterations model size 2 was selected 99 times and model size 3
once. Model with 2 variables was always `abdomen` and `weight`. Model
with 3 variables had also `wrist`. When using the full data, model size
2 was also feasible based on the predictive performance estimates. It is
likely that in bootstrap iterations model size 2 was favored as
bootstrapped data has less information than the full data. The selection
process in projection predictive variable selection is very stable, the
variability can be estimated using the fast PSIS-LOO over the selection
process, and projection predictive variable selection consistently
provides similar predictive performance as the full model.
...
```{r}
  ...
  print(paste('PSIS-LOO RMSE selected propred model: ', round(loormse_proj,1)))
```
...
considering we dropped from 13 covariates to 3 covariates.
...
Add 87 variables which are random normal distributed noise and thus are
...
Variable selection with `projpred has selected 3 variables from which 0
